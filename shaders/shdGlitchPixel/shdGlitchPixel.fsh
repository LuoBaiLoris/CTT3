...
void main()
{
	...
	nh *= glitch + rnd;
	float r = texture2D(gm_BaseTexture, uv+vec2(nh, 0.08)*nh).r;
	float g = texture2D(gm_BaseTexture, uv+vec2(nh-0.07, 0.0)*nh).g;
	float b = texture2D(gm_BaseTexture, uv+vec2(nh, 0.0)*nh).b;
